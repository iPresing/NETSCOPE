...
Retourne un flux `text/csv` (charset UTF-8, BOM) construit à la volée via
`CsvExporter.generate_anomalies_csv` et servi avec `stream_with_context`
pour préserver le contexte d'application Flask tout en évitant l'accumulation
en mémoire côté serveur (critique sur Raspberry Pi Zero).
...
from app.services.export_service import get_csv_exporter
...
def _filename_for(capture_id: str) -> str:
    """Produit le nom de fichier `netscope-anomalies-{capture_id}-{timestamp}.csv`."""
    safe_id = _sanitize_for_header(capture_id)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    return f"netscope-anomalies-{safe_id}-{stamp}.csv"
